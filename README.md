...
cd ~/dev/harness-kit
uv run harness-kit plan
uv run harness-kit apply
```
...
- `uv run harness-kit plan [--harness claude|pi|all]` prints the desired deployment without changing anything.
- `uv run harness-kit apply [--harness claude|pi|all] [--dry-run]` renders agents, installs Pi dependencies and package registration when selected, and creates only safe owned links. Add `--adopt-legacy` once to migrate links from the old `~/dev/skills` and `~/dev/pi-kit` checkouts.
