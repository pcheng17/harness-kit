...
- `uv run harness-kit install [--harness claude|pi|all] [--component skills|agents|instructions] [--dry-run]` renders selected agents, installs Pi dependencies and package registration only when Pi agents are selected, and creates only safe owned links. Old links must be removed manually.
